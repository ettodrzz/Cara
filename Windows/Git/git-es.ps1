...
Function Check-Paths
{
    ...
    if (($PathEnvVar -match [regex]::Escape("$Env:LocalAppData\Programs\Git\mingw64\bin")) -and ($PathEnvVar -match [regex]::Escape("$Env:LocalAppData\Programs\Moar")))
    {
        return $True
    }
}
...
{
    $PathEnvVar = [System.Environment]::GetEnvironmentVariable("Path", [System.EnvironmentVariableTarget]::User)
    $UpdatedPathEnvVar = (($PathEnvVar).Replace("$Env:LocalAppData\Programs\Git\mingw64\bin;", "")).Replace("$Env:LocalAppData\Programs\Moar;", "")
    [Microsoft.Win32.Registry]::SetValue("HKEY_CURRENT_USER\Environment", "Path", $UpdatedPathEnvVar, [Microsoft.Win32.RegistryValueKind]::ExpandString)
}
